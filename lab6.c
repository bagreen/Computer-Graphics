// painter's algorithm
// fill in polys, furthest first
// find center of mass of each polygon
// find distance from center to origin
// sort by z value
...
void draw_polygon(int num_objects) {
	...
	double distance;
        ...
        // print polygon
        if (obj_num == 0) G_rgb(0, 0.5, 0);
        if (obj_num == 1) G_rgb(0, 0, 0.5);
        G_fill_polygon(x_points, y_points, psize[obj_num][poly_num]);
       
        if (obj_num == 0) G_rgb(0, 1, 0);
        if (obj_num == 1) G_rgb(0, 0, 1);
